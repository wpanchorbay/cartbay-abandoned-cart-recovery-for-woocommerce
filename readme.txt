...
1. Recovery analytics overview — tracked, abandoned, and recovered carts, abandoned cart value, recovered revenue, and recovery rate, with 7/30/90-day filters.
2. Consent-based capture settings for both classic checkout and the WooCommerce Block Checkout.
3. The 3-step recovery email sequence with per-step delays you control.
4. Recovery email templates, edited through native WooCommerce email settings.
5. Email delivery health checks and the built-in Send Test Email tool.
6. The guided setup wizard for fast first-run configuration.
